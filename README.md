...
它利用可微渲染（Differentiable Rendering）技术，将高模（Reference Mesh）的**光影、材质和细节**精确地“烘焙”到低模（Base Mesh）的 Diffuse 贴图或者一组pbr贴图上。不同于传统的烘焙工具，DiffBake 通过梯度下降算法直接优化纹理像素，能够生成像素级对齐（Pixel-perfect）、边缘锐利且抗锯齿的高质量贴图。
...
| `--use_opt_pbr`     | `bool`    | `False`     | **是否启用 PBR 渲染**。  <br/>建议在 full_pbr 模式下设为 True，使优化过程考虑粗糙度和金属度对光照的影响。                                                                |
| `--bake_mode`       | `str`     | `full_pbr`  | **烘焙模式。**<br/>• `color_only`：  仅优化 Kd (Albedo)，导出时只包含 BaseColor。<br/>• `full_pbr`:优化 Kd, Ks, Normal，导出包含 BaseColor, ORM, Normal 贴图。 |
...
| `--smooth_weight`  | `float`   | `0.02`      | **平滑正则化权重**。如果需要保留更多砖块/噪点细节，建议降低至 0.005。                                                                                    |
...
| `--cam_views_top`    | `int`       | `64`         | **上半球相机数量**。增加此值可覆盖更多顶部细节。                                                        |
| `--cam_views_bottom` | `int`       | `16`         | **下半球相机数量**。增加此值可覆盖更多底部细节。                                                        |             |              |                                                                                   |
